...
typedef struct Colonne_s {
  couleur *bille;            /* Tableau dynamique de billes */
  struct Colonne_s *agauche;   /* Pointeur vers la Colonne de gauche */
  struct Colonne_s *adroite;   /* Pointeur vers la Colonne de droite */
} Colonne ;
...
typedef struct Grille_s {
  ...
  int fond_img_sprite;
  Colonne* prem;              /* Un pointeur sur la Colonne la plus à gauche. */
  Colonne* courante;          /* la Colonne sur laquelle on agit à un instant t. */
